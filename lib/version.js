...
function runUpdate(version) {
  console.log(version ? `Updating stk to version ${version} ...` : 'Updating stk ...');
  // npm >=12 defaults allow-git=none, which blocks git-spec installs; opt in for this call.
  const r = spawnSync('npm', ['i', '-g', '--allow-git=all', REPO_SPEC], {
    encoding: 'utf8',
    shell: process.platform === 'win32', // npm is npm.cmd on Windows
  });
  try { fs.unlinkSync(CACHE); } catch { /* ignore */ }
  if (r.status === 0) {
    console.log(version ? `stk updated to version ${version}.` : 'stk updated.');
  } else {
    if (r.stderr) process.stderr.write(r.stderr);
    console.error('stk: update failed.');
  }
